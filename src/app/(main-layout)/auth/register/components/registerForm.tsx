...
import { IFormRegister, IRegisterResponse } from "../_interface/register";
import { useMutation } from "@apollo/client";
import { MUTATION_REGISTER } from "@/graphql/auth";
...
function RegisterForm() {
  ...
  const [mutationRegister] = useMutation<IRegisterResponse>(MUTATION_REGISTER);
  ...
  const onSubmit = async (data: IFormRegister) => {
    try {
      const response = await mutationRegister({
        variables: {
          shopId:SHOP_ID,
          userRegister: {
      ...
      if (response.data && response.data?.register.success) {
        message.success("Đăng nhập thành công") // Optionally show a message error
        setTimeout(() => {
          router.push("/auth/login")
        }, 1500);
      } else {
        const errorMessage = response.data?.register.message ?? "Lỗi đăng ký";
        // Set the error on the email field using setError
        setError("email", {
          type: "manual",
          message: errorMessage,
        });
        message.error(errorMessage); // Optionally show a message error
      }
      // Handle successful registration, e.g., redirect to a different page or show a success message
    } catch (error) {
    ...
    >
      ...
      <div className="input_wrapper">
        <div className="flex justify-between w-full">
          ...
          <Link href={Routers.home.pathHome}>Quên mật khẩu?</Link>
        </div>
