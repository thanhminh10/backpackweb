...
function LoginForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const { data: loginData } = await mutationLogin({
        variables: { userLogin: { email, password }, shopId: SHOP_ID },
      });
      ...
      const token = loginData?.loginShop.data?.token;

      if (token) {
        localStorage.setItem("token", token);
        const userInfo = await fetchUserInfo();
        localStorage.setItem("userInfo", JSON.stringify(userInfo));
        // Khôi phục trạng thái với makeVar
        const usercart = await fetchUserCart();
        userVar(userInfo);
        userCartVar(usercart)
        isLoggedInVar(true);
        ...
        setError("password", {
          type: "manual",
          message: loginData?.loginShop.message,
        });
        message.error(
          loginData?.loginShop.message ??
            "Đăng nhập thất bại vui lòng kiểm tra mật khẩu"
        );
      }
    } catch (error) {
