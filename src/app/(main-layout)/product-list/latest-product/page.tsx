...
import { GetLatestProductData } from "@/interfaces/product/product";
...
export default function LatestProductList() {
  ...
  const {
    ...
    variables: { pageIndex: page, pageSize: limit, sort: parseInt(getSort) ,shopId : SHOP_ID },
  });
  ...
  if (latestProductsLoading) return <Loading />;
  if (latestProductsError) return <p> Error</p>;
  const totalItems = latestProductsData?.products?.links?.totalItems ?? 0;
